...
function convertToHTML(jsx) {
  return jsx
    ...
    .replace(/const \w+ = \(\) => \(/g, '') // Remove function declaration
    .replace(/const \w+ = \(.*?\) => \(/g, '') // Remove function with params
    .replace(/^\);$/gm, '') // Remove closing parenthesis
...
console.log('Starting improved HTML generation...');
...
sections.forEach((section, index) => {
  ...
  try {
    ...
    // Extract JSX content (everything between return ( and final );)
    const returnStart = content.indexOf('return (');
    const lastCloseParen = content.lastIndexOf(');');

    if (returnStart !== -1 && lastCloseParen !== -1) {
      content = content.substring(returnStart + 8, lastCloseParen).trim();
    }
...
console.log('\nTotal content lines:', totalLines);
...
const htmlLines = htmlTemplate.split('\n').length;
console.log('\n========================================');
console.log('✓ HTML COMPLETO GENERADO EXITOSAMENTE!');
console.log('========================================');
...
console.log('  Líneas HTML totales:', htmlLines);
console.log('\n✓ ARCHIVO FUNCIONAL CON file:// protocol');
console.log('  Abre con DOBLE CLIC o en cualquier navegador!');
console.log('========================================\n');
